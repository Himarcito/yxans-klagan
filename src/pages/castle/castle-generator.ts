...
const ageTable = [
  ...
  { max: 66, text: `Después de la Niebla de Sangre (${d6()} años)`, stateMod: 0 }
];
...
const pickD66 = <T extends { max: number }>(table: T[], roll: number): T => table.find(r => roll <= r.max) || table[table.length - 1];
...
export const createRandomCastle = (): Castle => {
  ...
  const tAge = pickD66(ageTable, d66());
  
  let state = 'Bien conservado';
  let inhabMod = 2;
  const stateRoll = d6() + tAge.stateMod;
